		...
		for (int row = 0; row < width; ++row) {
			...
			for (int s = 0; s < samples && !skip; ++s) {
				...
				if (x < m_imageCanvas.width() && y < m_imageCanvas.height() && x >= 0 && y >= 0)
					if (m_imageCanvas.pixelColor(x, y).red() == m_default_pixel_color.x() && m_imageCanvas.pixelColor(x, y).green() == m_default_pixel_color.y() &&
					    m_imageCanvas.pixelColor(x, y).blue() == m_default_pixel_color.z())
						skip = true;
			}
			if (!skip)
