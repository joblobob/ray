...
export void updateParticleDensity(std::vector<Particle>& particleMap, std::vector<Cell>& gridCells)
{
	...
	auto solveParticleDensity = [&](const Particle& p) {
		...
		const unsigned int x0 = static_cast<unsigned int>((x - constants::halfCellHeight) * constants::fInvSpacing);
		...
		const unsigned int y0 = static_cast<unsigned int>((y - constants::halfCellHeight) * constants::fInvSpacing);
